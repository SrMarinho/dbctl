...
def status(ctx: typer.Context) -> None:
    ...
    def _do() -> None:
        ...
        if hook["installed"] and hook["ours"]:
            hook_line = f"installed, {'enabled' if hook['enabled'] else 'disabled'}"
        elif hook["installed"]:
            hook_line = "third-party hook present"
        else:
...
# --------------------------------------------------------------------------
# hook sub-app (post-checkout git hook management)
# --------------------------------------------------------------------------
...
hook_app = typer.Typer(
    ...
    invoke_without_command=True,
    add_completion=False,
    help="Manage the post-checkout git hook (branch switch -> dbctl use).",
)
...
def hook_install(
    ...
    force: bool = typer.Option(
        False,
        "--force",
        help="Overwrite a third-party post-checkout (backed up as post-checkout.bak).",
    ),
) -> None:
    ...
    def _do() -> None:
        ...
        if result.get("backup"):
            typer.echo(f"backed up the previous hook to {result['backup']}")
        typer.echo(f"hook {result['action']}: {result['path']}")
...
def hook_uninstall(ctx: typer.Context) -> None:
    ...
    def _do() -> None:
        ...
        elif result["action"] == "absent":
            typer.echo("no post-checkout hook to remove.")
        elif result["action"] == "kept":
            typer.echo(
                "post-checkout exists but was not generated by dbctl - "
                f"leaving it in place: {result['path']}"
            )
        else:
...
def hook_status(ctx: typer.Context) -> None:
    ...
    def _do() -> None:
        ...
        if info["installed"]:
            owner = "dbctl" if info["ours"] else "third-party"
            typer.echo(f"installed: yes ({owner})")
        else:
...
@hook_app.command("post-checkout", hidden=True)
def hook_post_checkout(
    ...
    """Runner called by git on every post-checkout (NOT for manual use).

    Golden rule: this command NEVER exits non-zero. It wraps everything -
    including config loading - in its own try/except and always ends with
    exit 0, so a checkout can never be broken by the hook.
    """
    try:
        cfg = _load_config(ctx, quiet=True)
